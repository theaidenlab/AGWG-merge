...
- [alignments.txt](https://www.dropbox.com/s/yo6cfh12amnegcv/alignments.txt?dl=0) and [overlaps_2D_input.txt](https://www.dropbox.com/s/l62zk9r32t9dber/overlaps_2D_input.txt?dl=0) - list of all pairwise alignments that fit the alignment score, overlap length, position and sequence identity criteria to be marked as alternative haplotypes and the same data in the 2D annotation format, as generated by the 3D-DNA merge module (see below);
- [overlaps_2D_asm.txt](https://www.dropbox.com/s/zae7wrlcp0du1xe/overlaps_2D_asm.txt?dl=0) and [overlaps_2D_asm_post_tiling.txt](https://www.dropbox.com/s/3thcy9vajyboozj/overlaps_2D_asm_post_tiling.txt?dl=0) - 2D track with positions of the same overlaps in genome-wide reference frame, as generated by 3D-DNA merge module, and the same track lifted to genome assembly after automatic tiling (see below);
- [AGWG.rawchrom_tiled.asm](https://www.dropbox.com/s/y2b6q3cy0itx3sr/AGWG.draft.rawchrom_tiled.asm?dl=0) - assembly file listing merge blocks -- chains of overlapping contigs organized based on alignment information -- as identified by the 3D-DNA merge module (see below);
- [AGWG.rawchrom_tiled.assembly](https://www.dropbox.com/s/4w4swbmfglvd72i/AGWG.draft.rawchrom_tiled.assembly?dl=0) - Juicebox Assembly Tools-compatible version of the same file (see below);
- [AGWG.rawchrom_tiled.hic](https://www.dropbox.com/s/elq52skw6s1uub6/AGWG.rawchrom_tiled.hic?dl=0), [AGWG.rawchrom_tiled_0.hic](https://www.dropbox.com/s/amammuvavmn6z93/AGWG.rawchrom_tiled_0.hic?dl=0), [AGWG.rawchrom_tiled_30.hic](https://www.dropbox.com/s/lbq6o8vcpql9rap/AGWG.rawchrom_tiled_30.hic?dl=0) - Hi-C contact maps as generated by the 3D-DNA pipeline visualization module (see below), thresholded at mapping quality 1, 0 and 30, respectively (see below);
- [AGWG.rawchrom_tiled_asm.scaffold_track.txt](https://www.dropbox.com/s/sho6fkq2zcjj7xd/AGWG.rawchrom_tiled_asm.scaffold_track.txt?dl=0) and [AGWG.rawchrom_asm.superscaf_track.txt](https://www.dropbox.com/s/re3os6yl21i0z34/AGWG.rawchrom_tiled_asm.superscaf_track.txt?dl=0) - 2D annotation tracks for this step as generated by 3D-DNA visualization module (see below).
...
```
For interactive review of results associated with step #5 compare http://bit.ly/2JIu3Z7 and http://bit.ly/2Hn3kmw. The links show, respectively, the contact maps before tiling ([AGWG.rawchrom_0.hic](https://www.dropbox.com/s/fmqra3xglgrd5vp/AGWG.rawchrom_0.hic?dl=0)) and after tiling ([AGWG.rawchrom_tiled_0.hic](https://www.dropbox.com/s/amammuvavmn6z93/AGWG.rawchrom_tiled_0.hic?dl=0)), with two sets of annotations overlaid over each of the maps. As before, the heatmaps indicate the frequency of contact between pairs of loci as measured by the Hi-C experiment, on a scale from white to red; pure white indicates no observed contacts, whereas pure red indicates a maximum number of contacts which is indicated in the left top corner. The same scale is used for both maps by default.

On the left panel of each of the interactive figures, 2D square annotations along the diagonal indicate the boundaries of contigs, encoded in [AGWG.rawchrom_asm.scaffold_track.txt](https://www.dropbox.com/s/bwnmosxusav5jwb/AGWG.rawchrom_asm.scaffold_track.txt?dl=0) and [AGWG.rawchrom_tiled_asm.scaffold_track.txt](https://www.dropbox.com/s/sho6fkq2zcjj7xd/AGWG.rawchrom_tiled_asm.scaffold_track.txt?dl=0), respectively. On the right, the same area of the map is shown with overlaid annotations of tentative overlaps as identified by pairwise LASTZ alignments ([overlaps_2D_asm.txt](https://www.dropbox.com/s/zae7wrlcp0du1xe/overlaps_2D_asm.txt?dl=0) and [overlaps_2D_asm_post_tiling.txt](https://www.dropbox.com/s/3thcy9vajyboozj/overlaps_2D_asm_post_tiling.txt?dl=0)).
...
- [AGWG.rawchrom_tiled.review.asm](https://www.dropbox.com/s/shfnht2r6zt45pf/AGWG.rawchrom_tiled.review.asm?dl=0) - the curated version of the file [AGWG.rawchrom_tiled.asm](https://www.dropbox.com/s/y2b6q3cy0itx3sr/AGWG.draft.rawchrom_tiled.asm?dl=0) listing chains of overlapping contigs;
